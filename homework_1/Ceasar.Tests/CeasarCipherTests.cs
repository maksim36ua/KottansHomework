...
namespace Ceasar.Tests
{
    public class CeasarCipherTests
    {
        ...
        [Test]
        public void ClassConstructor_InappropriateOffsetValue_ThrowsArgumentOutOfRangeException()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                var cipher = new CeasarCipher(offset: 100);
            });

            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                var cipher = new CeasarCipher(offset: 94);
            });

            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                var cipher = new CeasarCipher(offset: -10);
            });
        }
    }
}
